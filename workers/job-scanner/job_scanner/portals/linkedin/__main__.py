"""CLI: python -m job_scanner.portals.linkedin --keyword "python developer" --location bangalore"""
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Scrape LinkedIn public job search results")
    ...
    parser.add_argument("--pages", "-p", type=int, default=1, help="Max pages (~25 jobs each)")
    parser.add_argument(
        "--headless",
        action="store_true",
        help="Run headless (may be blocked more often)",
    )
    parser.add_argument(
        "--json",
        action="store_true",
